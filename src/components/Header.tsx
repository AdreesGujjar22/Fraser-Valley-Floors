  ...
  return (
    <header
      className={`sticky top-0 z-50 w-full transition-all duration-300 ${
        scrolled ? "bg-background/95 backdrop-blur border-b border-border" : "bg-background/70 backdrop-blur"
      }`}
    >
      <div className="container-x flex h-16 items-center justify-between gap-4 md:h-20">
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <a href={site.phoneHref} className="flex items-center gap-2 text-sm font-semibold hover:text-primary transition-colors">
      ...
      {open && (
        <div className="lg:hidden border-t border-border bg-background max-h-[calc(100vh-4rem)] overflow-y-auto">
          <div className="container-x py-4 flex flex-col gap-1">
            ...
            <MobileGroup label="Services" open={openMobile === "s"} onToggle={() => setOpenMobile(openMobile === "s" ? null : "s")}>
              <MobileLink to="/services" onClick={() => setOpen(false)}>All Services</MobileLink>
              <div className="mt-2 mb-1 px-3 text-xs uppercase tracking-widest text-primary">Concrete Coatings</div>
              {coatingServices.map((s) => (
                <MobileLink key={s.slug} to="/services/$slug" params={{ slug: s.slug }} onClick={() => setOpen(false)}>
                  {s.name}
                </MobileLink>
              ))}
              <div className="mt-2 mb-1 px-3 text-xs uppercase tracking-widest text-primary">Flooring</div>
            ...
            <MobileGroup label="Locations" open={openMobile === "l"} onToggle={() => setOpenMobile(openMobile === "l" ? null : "l")}>
              {locations.map((l) => (
                <MobileLink key={l.slug} to="/locations/$city" params={{ city: l.slug }} onClick={() => setOpen(false)}>
                  {l.city}
                </MobileLink>
              ))}
            </MobileGroup>
            <MobileLink to="/contact" onClick={() => setOpen(false)}>Contact</MobileLink>
            <a href={site.phoneHref} className="mt-3 flex items-center gap-2 px-3 py-2 text-sm font-semibold">
    ...
    <div className="group relative">
      ...
      <div className="invisible absolute left-1/2 top-full -translate-x-1/2 pt-3 opacity-0 transition-all duration-200 group-hover:visible group-hover:opacity-100">
        <div className="w-[640px] rounded-lg border border-border bg-popover p-6 shadow-2xl grid grid-cols-2 gap-6">
          <div>
            <div className="mb-3 text-xs font-bold uppercase tracking-widest text-primary">Concrete Coatings</div>
            <ul className="space-y-1">
              {coatingServices.map((s) => (
                <li key={s.slug}>
                  <Link to="/services/$slug" params={{ slug: s.slug }} className="block rounded px-2 py-1.5 text-sm hover:bg-surface-elevated hover:text-primary transition-colors">
                    {s.name}
                  </Link>
                </li>
              ))}
          ...
          <div>
            <div className="mb-3 text-xs font-bold uppercase tracking-widest text-primary">Flooring</div>
            <ul className="space-y-1">
              {flooringServices.map((s) => (
                <li key={s.slug}>
                  <Link to="/services/$slug" params={{ slug: s.slug }} className="block rounded px-2 py-1.5 text-sm hover:bg-surface-elevated hover:text-primary transition-colors">
    ...
    <div className="group relative">
      ...
      <div className="invisible absolute left-1/2 top-full -translate-x-1/2 pt-3 opacity-0 transition-all duration-200 group-hover:visible group-hover:opacity-100">
        <div className="w-56 rounded-lg border border-border bg-popover p-3 shadow-2xl">
          <Link to="/locations" className="block rounded px-2 py-1.5 text-sm font-semibold hover:bg-surface-elevated hover:text-primary">
            All Service Areas
          </Link>
          <div className="my-1 h-px bg-border" />
          {locations.map((l) => (
            <Link key={l.slug} to="/locations/$city" params={{ city: l.slug }} className="block rounded px-2 py-1.5 text-sm hover:bg-surface-elevated hover:text-primary transition-colors">
              {l.city}
            </Link>
          ))}
        </div>
